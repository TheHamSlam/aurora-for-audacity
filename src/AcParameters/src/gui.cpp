...
//----------------------------------------------------------------------------
// --- Useful methods ---
void AcParametersShowDialog::UpdateCellValue(const int nPar,  const int  nRow,
		                                     const int nBand, const bool bInit)
{
	 wxString wxszStr;
	 if(m_paResults[m_nCurrentChannel].IsParameterValid(nPar, nBand))
	 {
		wxszStr.Printf(wxT("%f"),
				       m_paResults[m_nCurrentChannel].GetParameter(nPar, nBand));
	 }
	 else
	 {
		wxszStr.Printf(wxT("-.-"));
...
//   wxString wxszStr;
   for(nCol = 0; nCol < int(m_pAp->GetBandsCount()); nCol++)
   {
		...
		for(nRow = 0, nPar = 0; nPar < AcParameters::AP_ST1; nRow++, nPar++)
		{
			 //printf("Updating parameter %d\n", nRow);
...
void AcParametersShowDialog::UpdatePlot(const bool bReplaceData)
{
   ...
   if( m_bIsScopeOn )
   {
	    if(bReplaceData)
	    {
			m_pDecayPlot->ReplaceSerie(0, m_paAudioTracks->Item(m_nCurrentChannel));
			m_pDecayPlot->ReplaceSerie(1, m_paDecay[m_nCurrentChannel]);
	    }

	    if(m_paResults[0].GetBandsCount() > 12)
	    {
	    	//One-Third Octave bands
			if(m_nCurrentBand != AcParameters::TO_BD_Lin)
			{
			   wxszMsg = ::AuroraGetThirdOctaveBandLabel(m_nCurrentBand);
			   if(m_nCurrentBand < AcParameters::TO_BD_A)
			   {  wxszMsg << wxT(" Hz band"); }
	    ...
	    else
	    {
			...
			{
			   wxszMsg = ::AuroraGetOctaveBandLabel(m_nCurrentBand);
...
void AcParametersShowDialog::CreateDecayPlot()
{
	...
	m_pPlot = new APDecayPlot(m_pwxpnlPlotDesk, ID_PLOT,
			                  wxDefaultPosition, wxSize(nWidth, nHeight));

	bPlotSizer->Add( m_pPlot, 1,
			         wxEXPAND|wxALIGN_CENTER_VERTICAL|wxALL|wxALIGN_CENTER_HORIZONTAL,
			         5 );
...
void AcParametersShowDialog::CreateBarPlot()
{
	...
	m_pPlot = new APBarPlot(m_pwxpnlPlotDesk, ID_PLOT,
			                wxDefaultPosition, wxSize(nWidth, nHeight));
...
//	m_pDecayPlot->GetLevelAxis()->SetRange(-m_pAp->GetdBRange(), 0.0, AFAxis::RF_LINEARdB); // TODO remove GetdBRange
	...
	pTds = new AFTimeDataSerie(m_paAudioTracks->Item(m_nCurrentChannel),
						       AFTimeDataSerie::HT_RMS);
	pTds->SetLabel(wxT("RMS"));
	pTds->SetLineColour(0,0,255);
	m_pDecayPlot->AddSerie(pTds);

	pTds = new AFTimeDataSerie(m_paDecay[m_nCurrentChannel],
...
void AcParametersShowDialog::SetupBarPlot()
{
	...
	adbSerie = new double [bandsCount];

	// ---> Bar plot
	switch(m_pAp->GetAnalysisType())
	{
	    case AcParameters::AP_OCTAVE_BANDS:
	    	m_pBarPlot->GetBandsAxis()->AddCategories(g_awxpchBandLabels,
	    			                                  bandsCount);

	        break;

	    case AcParameters::AP_ONE_THIRD_OCTAVE_BANDS:
	    	m_pBarPlot->GetBandsAxis()->AddCategories(g_awxpchThirdOctaveBandLabels,
	    			                                  bandsCount);
	    	break;
	}
	...
	for(int nParam = 0; nParam < AcParameters::N_PARAMETERS; nParam++)
	{
	   ...
	   m_pBarPlot->AddSerie(adbSerie, bandsCount, col, col);
...
void AcParametersShowDialog::OnOk(wxCommandEvent &event)
{
	...
	{
	    case AcParameters::AP_OCTAVE_BANDS:
	    	bandLabels = (wxChar*)g_awxpchBandLabels;
	        break;

	    case AcParameters::AP_ONE_THIRD_OCTAVE_BANDS:
	    	bandLabels = (wxChar*)g_awxpchThirdOctaveBandLabels;
	    	break;
	}
	...
	for(int nChnl = 0; nChnl < int(m_pAp->GetChannelsCount()); nChnl++)
	{
		...
		if(m_nCurrentBand != BD_Lin)
		    wxszName << wxT("_") << bandLabels[m_nCurrentBand];
		else
		    wxszName << wxT("_not");
...
void AcParametersShowDialog::Init()
{
	...
	if(m_pAp->GetAnalysisType() == AFAcousticalParameters::AP_ONE_THIRD_OCTAVE_BANDS)
	{
	    int nNeededCols = m_paResults[0].GetBandsCount() - pGrd->GetNumberCols();
		pGrd->AppendCols(nNeededCols); // should be 20 always...

		for(int col = 0; col < pGrd->GetNumberCols(); col++)
		{
		    pGrd->SetColLabelValue(col, AuroraGetThirdOctaveBandLabel(col));
		}
	}
    ...
    m_bIsScopeOn(true)
{
	...
	RestoreGeometry();
	UpdatePlot();
	//m_pDecayPlot->Refresh(); // TODO check start plot: not correctly scaled
