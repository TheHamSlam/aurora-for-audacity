...
void SCSignalWindows::BartlettHann()
{
    ...
    {
        m_smpWindow[n] = 0.62 - 
                         0.48 * fabs(n / AFSample(m_unLength - 1) - 0.5) -
                         0.38 * cos(2.0 * M_PI * n / AFSample(m_unLength-1));
        n++;
    }
}
...
void SCSignalWindows::Bartlett()
{
    ...
    while(n < m_unLength)
    {
        m_smpWindow[n] = 1.0 - fabs( (AFSample(n) - (m_unLength - 1.0)/2.0 ) /
                                                 ((m_unLength - 1.0)/2.0) );        
        n++;
    }
}
...
void SCSignalWindows::Gaussian(const double sigma, const double p)
{
    ...
    while(n < m_unLength)
    {
        m_smpWindow[n] = AFSample(exp( -0.5 *
...
void SCSignalWindows::Hamming(const double alpha)
{
    ...
    while(n < m_unLength)
    {
        m_smpWindow[n] = AFSample(alpha - beta * cos((2.0*M_PI*n) /
                                          double(m_unLength - 1)));
        n++;
    }    
}
...
{    
    ...
    while(n < m_unLength)
    {
        m_smpWindow[n] = AFSample(0.5 * (1.0 - cos((2.0*M_PI*n) /
                                         double(m_unLength - 1)) ));
        n++;
    }    
}
...
void SCSignalWindows::Kaiser(const double beta)
{
    ...
    while(n < m_unLength)
    {
        ...
        m_smpWindow[n] = AFSample((I0 * beta * sqrt(1.0 - pow(((2.0*n)/
                                                      (N - 1)) - 1.0, 2.0))) / 
                                  (I0 * beta));
        n++;
    }
...
void SCSignalWindows::Triang()
{
    ...
    while(n < m_unLength)
    {
        m_smpWindow[n] = AFSample(1.0 - fabs( (double(n) - (m_unLength - 1.0)/2.0 ) /
                                                           ((m_unLength + 1.0)/2.0) ));
        n++;
    }
}
...
void SCSignalWindows::Create(unsigned int M, const int type, double p1, double p2)
{
    if(M != m_unLength)
    {
        Destroy();
        m_unLength = M;
        m_smpWindow = new AFSample [m_unLength];
    }
