...
void SCFirWin::Pad(const int M, const bool center)
{
    ...
    AFSample* yt = new AFSample [M];
    memset(yt, 0, M * sizeof(AFSample));

    int ofs = (center) ? (M-m_nNtaps)/2 : 0;
    memcpy(yt + ofs, m_adbTaps, m_nNtaps*sizeof(AFSample));
...
bool SCFirWin::Process()
{
    ...
    memset(m_adbTaps, 0, sizeof(AFSample)*m_nNtaps);
    ...
    {        
        ...
        for(k = 0; k < m_nNtaps; k++)
        {
            m_adbTaps[k] -= AFSample(lsinc[k] * m_vdbCutoffs[i]);
            m_adbTaps[k] += AFSample(rsinc[k] * m_vdbCutoffs[i+1]);
        }

    }
    
    AFSample* pWin = m_Window.Get();
                      ...
                      const double fNyq)
{
    ...
    Destroy();
    m_adbTaps = new AFSample [m_nNtaps];
