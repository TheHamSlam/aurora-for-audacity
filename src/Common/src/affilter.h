...
public:
   ...
   private:
	...
	void IIR(AFAudioTrack& dst, AFAudioTrack& src,
  ...
  public:
	...
	/// \param fQ - Q factor.
	void Filter(AFAudioTrack& dst, AFAudioTrack& src,
			    const int type, const double f0, const double fQ);

	/// A-weigthed filter.
	/// \param dst - Destination data container
	/// \param src - Source data container
	void AFilter(AFAudioTrack& dst, AFAudioTrack& src); //... bool preserve = true);

	/// High-pass filter, cutoff 20 Hz.
	/// \param dst - Destination data container
	/// \param src - Source data container
	void LFilter(AFAudioTrack& dst, AFAudioTrack& src);

	/// ISO 8042 filter. Used in accelerometer measures for the generic axis
	/// ponderation.
	/// \param dst - Destination data container
	/// \param src - Source data container
	void ISO_8041_Filter(AFAudioTrack& dst, AFAudioTrack& src);

	/// ITU P56 filter. Pass band filter 160 - 5500 Hz
	/// \param dst - Destination data container
	/// \param src - Source data container
	void ITU_P56_Filter(AFAudioTrack& dst, AFAudioTrack& src);

	/// UNI 9916 filter. Low pass, cutoff 250 Hz. Used in vibration measures.
	/// \param dst - Destination data container
	/// \param src - Source data container
	void UNI_9916_Filter(AFAudioTrack& dst, AFAudioTrack& src);
	...
	void OctaveFilter(AFAudioTrack& dst, AFAudioTrack& src, const double dbFc);

	/// DC component remover.
	/// \param track - Destination data container
	void RemoveMean(AFAudioTrack& track);
