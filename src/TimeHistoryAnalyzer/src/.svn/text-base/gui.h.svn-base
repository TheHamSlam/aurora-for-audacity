...
//----------------------------------------------------------------------------
// THAGridCellColoredRectRenderer
//----------------------------------------------------------------------------
...
class TimeHistoryAnalyzerFrame : public THADialog
{
  private:
    ...
    wxColour m_wxcolPanelsBckgnd;

    // --- Useful methods ---
    void UpdateResults();

    void InitColumns(const int nStartCol = 0, const int nEndCol = -1);
    void FillColumn(const int nCh, const int nChnls, wxString& wxszLabel);
